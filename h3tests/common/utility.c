...
void geoToStringDegs(const LatLng *p, char *str) {
    sprintf(str, "(%.9lf, %.9lf)", radsToDegs(p->lat), radsToDegs(p->lng));
}
...
void iterateAllDirectedEdgesAtRes(int res, void (*callback)(H3Index)) {
    for (IterCellsResolution iter = iterInitRes(res); iter.h;
        ...
        originToDirectedEdges(iter.h, edges);
...
void randomGeo(LatLng *g) {
    ...
    g->lat = degsToRads((((float)rand() / (float)(RAND_MAX)) * 180.0) - 90.0);
    g->lng = degsToRads((float)rand() / (float)(RAND_MAX)) * 360.0;
}
